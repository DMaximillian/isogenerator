...
ENTRYPOINT ["sh", "-c", "make output/${IMAGE_NAME}-${IMAGE_TAG}.iso ARCH=${ARCH} VERSION=${VERSION} IMAGE_REPO=${IMAGE_REPO} IMAGE_NAME=${IMAGE_NAME} IMAGE_TAG=${IMAGE_TAG} VARIANT=${VARIANT} WEB_UI=${WEB_UI} ENROLLMENT_PASSWORD=${ENROLLMENT_PASSWORD} SECURE_BOOT_KEY_URL=${SECURE_BOOT_KEY_URL}"]
